...
router.get("/clockinbaby/:id", async (req, res) => {
  try {
    ...
    const clockedInBabies = await babiesregistration.find({
      status: "ClockedIn",
    });
...
router.post("/clockinbaby", async (req, res) => {
  try {
    ...
    const updatedBaby = await babiesregistration.findByIdAndUpdate(
      babyId,
      { status: "ClockedIn" },
      { new: true }
    );
...
// Handle clocking in a baby
router.post("/clockoutbaby/:id", async (req, res) => {
  try {
    const babyId = req.params.id;
    const updatedBaby = await babiesregistration.findByIdAndUpdate(
      babyId,
      { status: "Clockedoutbaby" },
      { new: true }
    );
    if (!updatedBaby) {
      return res.status(404).send("Baby not found");
...
clo  } catch (error) {
    console.log("Error clocking out baby:", error);
    res.status(500).send("Unable to clock out baby");
  }
});
...
router.post("/clockinbaby/:id", async (req, res) => {
  try {
    ...
    await babiesregistration.findOneAndUpdate({ _id:baby_Id }, update);
...
router.get("/clockedoutbabies", async (req, res) => {
  try {
    const babies = await babiesregistration.find({ status: "Clockedoutbaby" });
    res.render("clockouttable", { babies });
...
router.post("/babiesUpdate/:id", async (req, res) => {
  try {
    await babiesregistration.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
    res.redirect("/babieslist");
  } catch (error) {
